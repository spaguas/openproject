...
RSpec.describe AddViewAllPrincipalsPermissionToExistingRoles, type: :model do
  ...
  describe "up migration" do
    ...
    context "when project roles have manage_members permission" do
      ...
      it "does not assign the migration role to groups with manage_members" do
        group_with_manage_members = create(:group)
        create(:member, project:, principal: group_with_manage_members, roles: [project_role])

        ActiveRecord::Migration.suppress_messages { described_class.migrate(:up) }

        expect(group_with_manage_members.members.where(project: nil)).to be_empty
      end
    end
